...
# protobuf compiler, python venv tooling) and refreshes each service's deps.
set -euo pipefail
...
# ── 2. System packages ──
# PostgreSQL (server + client) backs the whole app; python3-venv builds the
# isolated backtest-worker virtualenv; protobuf-compiler backs proto/gen.sh.
NEED_APT=0
...
if [[ "$NEED_APT" == "1" ]]; then
  echo "==> Installing system packages (postgresql, python3-venv, protobuf-compiler)"
  sudo apt-get update -qq
  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
    postgresql postgresql-client python3-venv python3-dev build-essential protobuf-compiler
fi
